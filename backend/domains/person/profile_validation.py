...
def validate_person_profile(profile: PersonProfile) -> None:
    ...
    if profile.date_of_birth > today:
        raise InvalidPersonProfileError("Das Geburtsdatum darf nicht in der Zukunft liegen.")

    if not 100 <= profile.height_cm <= 250:
        raise InvalidPersonProfileError("Die Größe muss zwischen 100 und 250 cm liegen.")
    ...
    for weight in (
        ...
        if weight is not None and (not isfinite(weight) or not 0 < weight <= 500):
    ...
    if profile.training_goal == TrainingGoal.WEIGHT_LOSS:
        if profile.start_weight_kg is None or profile.target_weight_kg is None:
            raise InvalidPersonProfileError(
                "Für Abnehmen sind Start- und Zielgewicht erforderlich."
            )

        if profile.target_weight_kg >= profile.start_weight_kg:
            raise InvalidPersonProfileError("Das Zielgewicht muss unter dem Startgewicht liegen.")
