...
export function createWorkoutEngine(
  ...
  if (
    ...
    throw new Error(
      'plannedDurationSeconds must be a positive integer',
    )
  }
...
export function applyWorkoutEngineEvent(
  ...
  switch (event.type) {
    case 'bike_movement_changed':
      return handleBikeMovementChanged(
        current,
        event.moving,
      )
...
function tickFinishWindow(
  ...
  if (current.bikeMoving === false) {
    const stoppedForSeconds =
      current.bikeStoppedForSeconds + 1

    if (
      stoppedForSeconds >=
      BIKE_STOP_DEBOUNCE_SECONDS
    ) {
      return {
        state: {
          ...current,
  ...
  const nextFinishWindowSeconds =
    current.finishWindowElapsedSeconds + 1

  const nextElapsedSeconds =
    current.elapsedSeconds + 1

  if (
    nextFinishWindowSeconds >=
...
function detectBikePause(
  ...
  if (current.bikeMoving !== false) {
    if (
      current.bikeStoppedForSeconds === 0
    ) {
      return null
    }
 ...
 */
export function shouldPlayWorkoutVideo(
  state: WorkoutEngineState,
): boolean {
...
export function shouldCountWorkoutTime(
  state: WorkoutEngineState,
): boolean {
  return isActiveState(state.state)
}
...
export function shouldShowFinishPrompt(
  state: WorkoutEngineState,
): boolean {
...
export function getFinishWindowRemainingSeconds(
  ...
  return Math.max(
    0,
    FINISH_WINDOW_SECONDS -
      state.finishWindowElapsedSeconds,
  )
}