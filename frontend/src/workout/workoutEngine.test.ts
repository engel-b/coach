...
function applyTick(
  ...
  return applyWorkoutEngineEvent(
    state,
    {
      type: 'tick',
    },
...
describe('workout engine', () => {
  it('starts in running state', () => {
    const state =
      createWorkoutEngine(1200)

    expect(state.state).toBe(
      'running',
    )

    expect(state.elapsedSeconds).toBe(
      0,
    )
  ...
  it('rejects invalid planned durations', () => {
    expect(() =>
      createWorkoutEngine(0),
    ).toThrow()

    expect(() =>
      createWorkoutEngine(-1),
    ).toThrow()

    expect(() =>
      createWorkoutEngine(1.5),
    ).toThrow()
  })
  ...
  it('enters finish window when planned duration is reached', () => {
    ...
    expect(
      transition.state.elapsedSeconds,
    ).toBe(3)
  ...
  it('exposes UI decisions through selectors', () => {
    ...
    expect(
      shouldPlayWorkoutVideo(state),
    ).toBe(true)

    expect(
      shouldCountWorkoutTime(state),
    ).toBe(true)

    state = applyTick(state)

    expect(
      shouldShowFinishPrompt(state),
    ).toBe(true)
